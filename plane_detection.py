...
# def segment_planes_ransac(pcd, distance_threshold=0.01, ransac_n=3, num_iterations=100, min_ratio = 0.01, min_num_points = 500):
#     planes = []
#     rest = pcd
...
#     while True:
#         if len(rest.points) < 150 or len(planes) >= 10:
#         # if len(rest.points) < 150:
#             break
#         plane_model, inliers = rest.segment_plane(distance_threshold=distance_threshold,
#                                                   ransac_n=ransac_n,
#                                                   num_iterations=num_iterations)
        
#         if len(inliers) < total_points * min_ratio or len(inliers) < min_num_points:
#             break
...
def segment_planes_ransac(pcd, distance_threshold=0.01, ransac_n=3, num_iterations=100, min_ratio=0.01, min_num_points=1):
    planes = []
    rest = pcd
    rest.estimate_normals(search_param=o3d.geometry.KDTreeSearchParamHybrid(radius=0.05, max_nn=30))
    total_points = np.asarray(rest.points).shape[0]
...
def classify_planes(planes, cam_ori, angle_threshold=AngleThre, d_threshold=DistanceThre, max_area=3.5):
    ...
    for i in range(len(planes)):
        ...
        hull, _ = merged_pcd_legacy.compute_convex_hull()
